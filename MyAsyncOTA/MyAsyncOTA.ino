...
void setup(void) {
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(200, "text/plain", "Hi! This is Rheinturm OTA.");
  });

  // You can also enable authentication by uncommenting the below line.
  // ElegantOTA.setAuth("admin", "password");

  RheinturmOTA.setID("test-hardware");  // Set Hardware ID
  RheinturmOTA.setFWVersion("1.0.0");   // Set Firmware Version

  RheinturmOTA.setTitle("OTA Portal");  // Set OTA Webpage Title

  RheinturmOTA.begin(&server);    // Start ElegantOTA

  // ElegantOTA callbacks
  RheinturmOTA.onStart(onOTAStart);
  RheinturmOTA.onProgress(onOTAProgress);
  RheinturmOTA.onEnd(onOTAEnd);
